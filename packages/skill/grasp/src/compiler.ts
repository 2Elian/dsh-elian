...
import type { CompilationRequest, CompilationProposal, Fact, SkillEdge, SkillGraph, SkillNode } from './types.ts'

/** Compile selected skills into a validated typed DAG. */
export async function compileSkillGraph(request: CompilationRequest): Promise<SkillGraph> {
...
function deterministicProposal(request: CompilationRequest): CompilationProposal {
  return {
...
function dedupeNodes(nodes: readonly SkillNode[]): SkillNode[] {
...
function inferEdges(nodes: readonly SkillNode[], proposed: readonly SkillEdge[]): SkillEdge[] {
...
function removeLowConfidenceCycles(nodes: readonly SkillNode[], edges: readonly SkillEdge[]): SkillEdge[] {
  let result = [...edges]
  while (true) {
    const graph = { sourceId: '__source__', sinkId: '__sink__', nodes, edges: result, goalFacts: [] as Fact[] }
    const ordered = tryOrder(graph)
    if (ordered !== undefined) return result
    const removable = result.filter(edge => edge.type === 'order').sort((left, right) => left.confidence - right.confidence)[0]
    if (removable === undefined) throw new Error('compiler produced a cycle that cannot be repaired by removing a soft order edge')
    result = result.filter(edge => edge !== removable)
  }
}

function tryOrder(graph: { readonly nodes: readonly SkillNode[]; readonly edges: readonly SkillEdge[] }): string[] | undefined {
...
function connectTerminals(nodes: readonly SkillNode[], edges: readonly SkillEdge[], goals: readonly Fact[]): SkillEdge[] {
  const result = [...edges]
  const incoming = new Set(edges.map(edge => edge.to))
  const outgoing = new Set(edges.map(edge => edge.from))
  for (const node of nodes) if (!incoming.has(node.id)) result.push({ id: `source-${node.id}`, from: '__source__', to: node.id, type: 'order', confidence: 1 })
  for (const node of nodes) {
    const isGoal = goals.length === 0 || goals.some(goal => node.effects.includes(goal))
    if (!outgoing.has(node.id) || isGoal) result.push({ id: `sink-${node.id}`, from: node.id, to: '__sink__', type: 'order', confidence: 1 })
  }
  return result
}
