...
export function retrieveSkills(
  ...
  memory: readonly ExperienceRecord[] = [],
  ...
  const memoryPrior = new Map<string, number>()
  for (const item of relevantMemory.slice(0, 5)) {
    for (const skillId of item.record.skills) memoryPrior.set(skillId, Math.max(memoryPrior.get(skillId) ?? 0, item.score * (item.record.success ? 1 : 0.25)))
  }
  ...
  const selected: SkillCandidate[] = ranked.slice(0, topK).filter(skill => (scores.get(skill.id) ?? 0) >= minScore).map(skill => ({
    skill,
...
function jsDistance(left: readonly number[], right: readonly number[]): number {
  const epsilon = 1e-12
  const midpoint = left.map((value, index) => (value + (right[index] ?? 0)) / 2)
  const kl = (values: readonly number[], target: readonly number[]) => values.reduce((sum, value, index) => value <= 0 ? sum : sum + value * Math.log(value / Math.max(target[index] ?? 0, epsilon)), 0)
...
export function candidateEffects(candidates: readonly SkillCandidate[]): Set<Fact> {
